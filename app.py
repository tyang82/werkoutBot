...
def handle_workouts(data, addition, task):
    if len(data['attachments']) > 0:
        ...
        if found_attachment:  # append the poster to the list of names to be updated in the database
            ...
            num = add_to_db(names, addition, ids, task)
            if num == len(names):
                like_message(data['group_id'], data['id'])
...
def add_to_db(names, addition, ids, task): #add "addition" to each of the "names" in the db
    cursor = None
    conn = None
    num_commited = 0
    try:
        ...
        for x in range(0, len(names)):
            cursor.execute(sql.SQL(
                ...
                (str(addition), ids[x],))
            cursor.execute(sql.SQL(
                "UPDATE wreck_data SET  gym = gym+%s, last_post = now() WHERE id = %s"),
